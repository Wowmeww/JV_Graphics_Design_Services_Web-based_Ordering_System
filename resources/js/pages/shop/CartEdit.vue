...
const product = reactive(props.cartItem.option || props.cartItem.product);
product.category = props.cartItem.product.category;
...
function update() {
    form.patch(route('cart.update', props.cartItem.id));
}

function orderNow() {
    router.get(route('order.create', { items: [props.cartItem.id], from: 'cart' }));
}

function handleDelete() {
    router.delete(route('cart.destroy', { cartItem: props.cartItem?.id || null, from: 'edit' }));
}
                ...
                <div class="h-[32rem] flex-1 overflow-y-scroll">
                    ...
                    <div class="space-y-5 border-b-2 border-slate-300 pt-6 pb-5">
                        ...
                        <div class="flex flex-col gap-x-3 gap-y-2 sm:flex-row">
                            <ButtonPrimary :disabled="form.processing" variant="outline-danger" @click="handleDelete" label="Remove to Cart" />
                            <ButtonPrimary :disabled="form.processing" @click="update" variant="secondary" label="Update Cart" />
