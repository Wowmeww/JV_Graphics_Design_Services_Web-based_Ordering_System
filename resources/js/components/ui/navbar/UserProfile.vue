...
const user_avatar_path = computed(() => {
    if (user.value.avatar_url) {
        return user.value.avatar_url.includes('https') ? user.value.avatar_url : `/storage/${user.value.avatar_url}`;
    }
...
const styleClass = {
    ...
    dropdown:
        'absolute right-0 z-50 mt-4 min-w-56 text-base list-none bg-white divide-y divide-gray-100 rounded-sm shadow-sm dark:bg-gray-700 dark:divide-gray-600',
    dropdownButton:
        'flex w-full cursor-pointer justify-between px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-600 dark:hover:text-white',
};
</script>
    ...
    <div v-if="user" class="relative">
        ...
        <div v-show="isDropdownOpen" id="dropdown-user" :class="styleClass.dropdown">
            <Link
                ...
                class="block w-full px-4 py-3 text-left hover:bg-slate-100 dark:hover:bg-gray-600"
            >
            ...
            <div class="py-1" role="none">
                ...
                <button v-if="!user.is_admin" type="button" @click="openCart" :class="styleClass.dropdownButton" role="menuitem">
                    Cart
                    <i class="fa-solid fa-cart-shopping"></i>
                </button>
                <button v-if="!user.is_admin" type="button" @click="openWishlist" :class="styleClass.dropdownButton" role="menuitem">
                    Wishlist
                    <i class="fa-solid fa-heart"></i>
                </button>

                <Link as="button" type="button" :href="route('appearance.edit')" :class="styleClass.dropdownButton" role="menuitem">
